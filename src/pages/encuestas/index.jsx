...
import EncuestaGrid from "./EncuestaGrid";
const Encuestas = () => {
  ...
  const endPage = Math.min(
    page + 2,
    formularioMetaData.totalPages ? formularioMetaData.totalPages : 5
  );
    ...
    <div>
      ...
      {filler === "grid" && !isLoaded && (
        <div className="grid xl:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-5">
          {formulariosList.map((project, projectIndex) => (
            <EncuestaGrid project={project} key={projectIndex} />
          ))}
        </div>
      )}
          ...
          <nav aria-label="Page navigation example">
            <ul className="flex items-center -space-x-px h-10 text-base">
              ...
              <li>
                <button
                  onClick={() => setPage(page + 1)}
                  disabled={!formularioMetaData.hasNextPage}
